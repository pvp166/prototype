...
class Woocommerce_Pay_Per_Post_Restrict_Content {
    ...
    */
    public function check_if_admin_call() : bool {
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_admin_call  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        if ( is_admin() || !$this->post_id ) {
            Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_admin_call  - Is an admin call' );
            return true;
        }
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_admin_call  - Is NOT an admin call' );
        return false;
    }
    ...
    public function check_if_admin_user_have_access() : bool {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_admin_user_have_access  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        // Check and see if admins are allowed to view protected content.
        if ( $admins_allowed_access && is_super_admin() ) {
            Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_admin_user_have_access  - Administrators HAVE access to all protected posts via settings' );
            return true;
        }
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_admin_user_have_access  - Administrators DO NOT HAVE access to all protected posts via settings' );
        return false;
    }

    public function check_if_user_role_has_access() : bool {
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_user_role_has_access  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        $allowed_user_roles = [];
    ...
    public function check_if_purchased() : bool {
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased  - Called. - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased  - Products associated with page = ' . print_r( $this->product_ids['product_ids'], true ) );
        ...
        if ( !empty( $this->product_ids['product_ids'] ) && count( (array) $this->product_ids['product_ids'] ) > 0 ) {
            foreach ( $this->product_ids['product_ids'] as $id ) {
                Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased  - Checking to see if user has purchased product #' . print_r( $id, true ) );
                if ( Woocommerce_Pay_Per_Post_Helper::can_use_woocommerce_subscriptions() && $this->integrations['woocommerce-subscriptions']->is_subscription_product( $id ) ) {
                    if ( wc_customer_bought_product( $this->current_user->user_email, $this->current_user->ID, trim( $id ) ) && $this->check_if_is_subscriber() ) {
                        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased  - User has purchased product id #' . trim( $id ) );
                        return true;
                    }
                } else {
                ...
                Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased  - User has NOT purchased product id #' . trim( $id ) );
            }
        }
        return false;
    }
    ...
    public function check_if_logged_in() : bool {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_logged_in  - ' . (( is_user_logged_in() ? 'true' : 'false' )) );
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_logged_in  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        return $logged_in;
    }
    ...
    public function check_if_has_access() : bool {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_has_access  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        switch ( $this->protection_type ) {
            ...
            case 'delay':
                ...
                Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_has_access  - Protection Type is Standard or Delayed' );
                Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_has_access  - Calling Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased()' );
                return $this->check_if_purchased();
            case 'page-view':
                Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_has_access  - Protection Type is Page View Protection' );
                return $this->has_access_page_view_protection__premium_only();
            case 'expire':
                Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_has_access  - Protection Type is Expiration Protection' );
    ...
    public function check_if_is_paid_memberships_pro_member() {
        ...
        if ( Woocommerce_Pay_Per_Post_Helper::can_use_paid_membership_pro() ) {
            ...
            Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_is_paid_memberships_pro_member  - Is the user a Paid Memberships Pro Member? - ' . (( $is_member ? 'true' : 'false' )) );
            return $is_member;
        }
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_is_paid_memberships_pro_member  - User is NOT a Paid Memberships Pro member, as Paid Memberships Pro is not installed.' );
        return false;
    }
    ...
    public function check_if_is_member() {
        ...
        if ( Woocommerce_Pay_Per_Post_Helper::can_use_woocommerce_memberships() ) {
            ...
            Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_is_member  - Is the user a WooMemberships Member? - ' . (( $is_member ? 'true' : 'false' )) );
            return $is_member;
        }
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_is_member  - User is NOT a member, as WooMemberships is not installed.' );
        return false;
    }
    ...
    public function check_if_is_subscriber() {
        ...
        if ( Woocommerce_Pay_Per_Post_Helper::can_use_woocommerce_subscriptions() ) {
            ...
            Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_is_subscriber  - Does the user have a valid subscription? - ' . (( $is_subscriber ? 'true' : 'false' )) );
            return $is_subscriber;
        }
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_is_subscriber  - User is NOT a subscriber, as WooSubscriptions is not installed.' );
        return false;
    }
    ...
    public function check_if_protected() : bool {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_protected  - product_ids - ' . var_export( $this->product_ids['product_ids'], true ) );
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_protected  - Page IS protected.' );
        return false;
    }
    ...
    public function check_if_post_contains_subscription_products() {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_post_contains_subscription_products  - Does Post/Page Contain Subscription Products? - ' . (( $post_has_subscription_product ? 'true' : 'false' )) );
    ...
    public function check_if_post_contains_membership_products() {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_post_contains_membership_products  - Does Post Contain Membership Products? - ' . (( $post_has_membership_product ? 'true' : 'false' )) );
    ...
    public function check_if_post_contains_paid_memberships_pro_membership_products() {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_post_contains_paid_memberships_pro_membership_products  - Does Post Contain Paid Membership Pro Membership Products? - ' . (( $post_has_membership_product ? 'true' : 'false' )) );
        return $post_has_membership_product;
    }

    public function check_if_should_show_paywall() : bool {
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_should_show_paywall  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        switch ( $this->protection_type ) {
            case 'standard':
            case 'page-view':
            case 'expire':
    ...
    public function can_user_view_content() : bool {
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/can_user_view_content  - BACKTRACE - Called From - ' . print_r( debug_backtrace()[1]['function'], true ) );
        $check_results = [];
        ...
        // if ( class_exists( 'Elementor\\Plugin' ) && \Elementor\Plugin::$instance->documents->get( get_the_ID() )->is_built_with_elementor() && !get_post_meta( get_the_ID(), WC_PPP_SLUG . '_product_ids', true ) ) {
        //     return true;
        // }
        return false;
    }
    ...
    protected function shortcode_pageview_status( $template ) {
        ...
        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/shortcode_pageview_status - Showing Pageview Status Warnings' );
    ...
    protected function shortcode_expiration_status( $template ) {
        ...
        if ( isset( $user_info['last_purchase_date'] ) ) {
            ob_start();
            Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/shortcode_expiration_status - Showing Expiration Status Warnings' );
            require Woocommerce_Pay_Per_Post_Helper::locate_template( $this->available_templates[$template], '', WC_PPP_PATH . 'public/partials/' );
            return ob_get_clean();
        }
        return false;
